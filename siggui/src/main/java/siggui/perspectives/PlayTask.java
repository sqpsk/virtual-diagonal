...
class PlayTask<Parameters extends PerspectiveParameters, Result> implements Runnable {

	PlayTask(AbstractPerspectiveController<Parameters, Result> model) {
		this.model = model;
	}
	...
	private void runThrows() throws Exception {
		...
		while (!Thread.interrupted()) {
			...
			final PerspectiveTask<Parameters, Result> task = model.newTask(p);
	...
	private final AbstractPerspectiveController<Parameters, Result> model;
