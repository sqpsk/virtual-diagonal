...
import siggui.perspectives.DefaultSaveImageAction;
import siggui.perspectives.PlayPanel;
...
import siggui.perspectives.AbstractPerspectiveView;

class TimeseriesView extends JPanel implements AbstractPerspectiveView {
	...
	TimeseriesView(TimeseriesController controller) {
		...
		popup.add(zplot.utility.SwingUtils.makeMenuItem("Save image", new DefaultSaveImageAction(plot, controller, "timeseries")));
	...
	private Plotter makePlotter(Color color) {
	...
	public void notifyNewResult(TimeseriesResultCache result) {
		...
		if (seriesMask == TimeseriesParameters.SERIES_IvQ) {
			Series series = InterleavedSeries.makeScatterSeries(result.getSeries(TimeseriesParameters.SERIES_IvQ));
			...
			if (seriesMask == TimeseriesParameters.SERIES_I_AND_Q) {
				Series seriesI = RegularSeries.makeFromBeginAndEnd(result.getSeries(TimeseriesParameters.SERIES_I), xBegin, xEnd);
				Series seriesQ = RegularSeries.makeFromBeginAndEnd(result.getSeries(TimeseriesParameters.SERIES_Q), xBegin, xEnd);
				scb.add("I", seriesI, makePlotter(Color.BLUE));
				scb.add("Q", seriesQ, makePlotter(Color.RED));
			} else {
				Series series = RegularSeries.makeFromBeginAndEnd(result.getSeries(seriesMask), xBegin, xEnd);
		...
		if (seriesMask == TimeseriesParameters.SERIES_IvQ) {
			dataTransformer = new SquareAxisTransformer();
			if (xAdaptive != null) {
				xAdaptive.newRange(sc.envelope().x());
				yAdaptive.newRange(sc.envelope().y());
				dataTransformer = new FixedScaleTransformer(dataTransformer, xAdaptive.getAxisRange(), yAdaptive.getAxisRange());
			}
		} else {
			dataTransformer = new FillCanvasTransformer();
			if (yAdaptive != null) {
				yAdaptive.newRange(sc.envelope().y());
				dataTransformer = new FixedScaleTransformer(dataTransformer, null, yAdaptive.getAxisRange());
			}
		}

		plot.init(sc, dataTransformer);
	...
	public void setEnabled(boolean enabled) {
		...
		SwingUtils.setEnabled(rangePanel, enabled);
		SwingUtils.setEnabled(controlPanel, enabled);
	}
