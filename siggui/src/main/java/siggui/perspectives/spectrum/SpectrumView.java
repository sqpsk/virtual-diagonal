...
class SpectrumView extends JPanel implements AbstractPerspectiveView {
	...
	SpectrumView(SpectrumController controller) {
		...
		popup.add(zplot.utility.SwingUtils.makeMenuItem("Save image", new DefaultSaveImageAction(plot, controller, "spectrum")));
	...
	private void notifyNewResult(SpectrumResult result) {
		...
		Series series = RegularSeries.makeFromBeginAndEnd(result.getPower(), xBegin, xEnd);
		...
		DataTransformer dataTransformer = new FillCanvasTransformer();
